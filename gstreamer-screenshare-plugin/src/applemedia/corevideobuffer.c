...
void gst_core_video_wrap_pixel_buffer(GstBuffer *buf, GstVideoInfo *info,
  ...
  if (CVPixelBufferIsPlanar(pixel_buf)) {
    ...
    mem = GST_MEMORY_CAST(
        gst_apple_core_video_memory_new_wrapped(gpixbuf, 0, size));
    gst_buffer_append_memory(buf, mem);
  }
  ...
  if (info) {
    gst_buffer_add_video_meta_full(buf, GST_VIDEO_FRAME_FLAG_NONE,
                                   GST_VIDEO_INFO_FORMAT(info), info->width,
                                   info->height, n_planes, offset, stride);
  }
}
